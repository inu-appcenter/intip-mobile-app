...
export function parseAcademicBasicInfo(responseBody: string): AcademicBasicInfo {
  ...
  ]) || deptMapped?.departmentName || departments[departmentCode || ''] || '';
  const collegeName = row['colgNm']?.trim() || deptMapped?.collegeName || '';
  ...
  return {
    ...baseResult,
    ...enriched,
    // 보존 우선순위: 유효한 값이 있는 경우 확인 불가로 덮어쓰지 않음
    enrollmentStatus: (enriched.enrollmentStatus && enriched.enrollmentStatus !== '확인 불가') ? enriched.enrollmentStatus : baseResult.enrollmentStatus,
    departmentName: (enriched.departmentName && enriched.departmentName !== '확인 불가') ? enriched.departmentName : baseResult.departmentName,
    collegeName: (enriched.collegeName && enriched.collegeName !== '확인 불가') ? enriched.collegeName : baseResult.collegeName,
  };
}
